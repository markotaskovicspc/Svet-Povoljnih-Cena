...
- Deployment naming: use an opaque, random, lowercase letter-only label for commits that trigger deployments (for example `qmvzpk`). Do not use descriptive deployment names such as `fix orders`.
