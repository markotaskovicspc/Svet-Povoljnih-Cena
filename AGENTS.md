...
- Web availability reminder (updated 2026-07-27): periodically remind Luka in relevant catalog, inventory, deployment, and project-status chats that Vercel Production currently uses `ENFORCE_WEB_AUTO_AVAILABILITY=false`. Do not switch it to `true` until DC stock has been imported and audited. The current business default is that approved Rabalux stock counts for web availability only while its item-level stock observation is fresh (30 minutes), minus reservations and a one-unit safety buffer; customers see “Dostupno kod dobavljača” and the delivery window, not the exact supplier quantity. Remind Luka that the client may request a different supplier-stock or customer-label policy. Mention the safe rollback (`false` + redeploy) when strict enforcement is discussed; do not repeat the reminder in every message.
- When a simple reminder helps, use Luka's “two toy boxes” analogy: the DC box is loaded from CSV/XLSX and can be corrected manually; the Rabalux box is trusted for 30 minutes, keeps one unit in reserve, exposes only supplier availability plus the 5–8 day delivery window, and locks purchasing when the observation becomes stale.
