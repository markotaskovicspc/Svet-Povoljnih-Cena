...
test.describe("Admin analitika reklamacija", () => {
  ...
  test("admin vidi tačne metrike, top artikle i funkcionalne filtere", async ({
    ...
    await test.step("operater može ručno da evidentira reklamaciju samo za isporučenu stavku", async () => {
      ...
      await expect
        .poll(() => db.reclamation.count({ where: { description } }))
        .toBe(1);

      const created = await db.reclamation.findFirstOrThrow({
        where: { description },
        include: { events: true },
      });
      expect(created.type).toBe("KVAR");
      expect(created.request).toBe("ZAMENA");
      expect(created.orderId).toBeTruthy();
      expect(
        created.events.some(
          (event) =>
            event.actorId && event.note === "Reklamacija ručno uneta u administraciji",
        ),
      ).toBe(true);

      await db.reclamation.delete({ where: { id: created.id } });
    });
