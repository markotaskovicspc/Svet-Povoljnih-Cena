...
export function compactMail(mail) {
  return {sender:mail.from?.value?.[0]?.address?.toLowerCase(),to:mail.to?.value?.map(a=>a.address.toLowerCase())??[],subject:String(mail.subject??'').slice(0,300),text:String(mail.text??'').slice(0,24000),messageId:mail.messageId??null,references:[].concat(mail.references??[],mail.inReplyTo??[]).filter(x=>/^<[^\r\n<>]{1,250}>$/.test(x)).slice(-8),attachments:mail.attachments?.map(a=>String(a.filename??'prilog').slice(0,150)).slice(0,12)??[]};
}
export function draftMessageId(key){return `<spc-draft-${hash(key)}@svetpovoljnihcena.rs>`;}
export async function composeDraft(message,body,key) {
  ...
  return new MailComposer({from:{name:'Podrška | Svet Povoljnih Cena',address:ADDRESS},to:message.sender,
    ...
    messageId:draftMessageId(key),inReplyTo:message.messageId||undefined,
...
export class EmailDraftWorker {
  ...
  async reconcile(c,row){
    ...
    for(const path of [this.drafts,this.sent].filter(Boolean)){
      await this.client.mailboxOpen(path,{readOnly:true});
      const found=await this.client.search({header:{'message-id':draftMessageId(row.id)}},{uid:true});
  ...
  async process(c,row){
    ...
    if(this.sent&&message.messageId){
    ...
    if(!draft){
      ...
      const result=await draftEmail({message,history,context,spc:this.spc,model:this.model});
      ...
      draft=this.encrypt({mime:(await composeDraft(message,result.body,row.id)).toString('base64')});
  ...
  async tick(){
    ...
    try{
      ...
      const rows=(await c.query("SELECT * FROM spc_email_drafts WHERE (status='pending' AND attempts<3) OR status IN ('prepared','appending') ORDER BY created_at LIMIT 5")).rows;
