...
await worker.start();
const server=await createHttpServer({store,worker,accounts,adminToken:process.env.CHAT_ADMIN_TOKEN,appSecret:process.env.META_APP_SECRET,verifyToken:process.env.META_VERIFY_TOKEN});
server.listen(Number(process.env.PORT??8080),'0.0.0.0',()=>console.log('SPC chatbot listening'));
for(const signal of ['SIGINT','SIGTERM'])process.on(signal,()=>{server.close(async()=>{await worker.stop();await store.close();process.exit(0);});});
