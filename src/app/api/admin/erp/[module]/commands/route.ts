...
async function createPickupBatchCommand(
  ...
  const rawProvider = String(input.provider ?? "").trim().toUpperCase();
  const provider =
    rawProvider === "MYGLS"
      ? "MYGLS"
      : rawProvider === "X EXPRESS" || rawProvider === "X_EXPRESS"
        ? "X_EXPRESS"
        : null;
  if (!provider) throw new Error("Izaberite MyGLS ili X Express.");
