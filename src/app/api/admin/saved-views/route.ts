...
export async function POST(request: Request) {
  ...
  const definition = getErpModuleDefinition(module);
  ...
  if (body?.isDefault === true) {
    await db.adminSavedView.updateMany({
      where: { adminUserId: admin.id, module, isDefault: true },
