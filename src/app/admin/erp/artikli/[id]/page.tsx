    ...
    async (_actorId, actionData: FormData) => {
      ...
      const created = await db.$transaction(async (tx) => {
        ...
        const nextPosition = Math.max(
          0,
          ...source.familyMembership.family.members.map(
        ...
        await propagateProductFamilySharedData(tx, sourceProductId, ["master"]);
            ...
            {product.familyMembership ? (
              <div className="mt-4 space-y-2">
                {product.familyMembership.family.members.map((member) => {
                  ...
                  const isPrimary =
                    member.productId ===
                    product.familyMembership?.family.primaryProductId;
                  return (
                    <div
                    ...
                    >
                      ...
                      <div className="min-w-0 text-xs">
                        ...
                        <p className="truncate text-ink-500">
                          {member.product.sku} ·{" "}
                          {price ? formatRsd(Number(price)) : "bez MPC"} · stanje{" "}
                ...
                >
                  ...
                  <Field label="Naziv boje">
                    <Input
                      name="label"
                      required
                      placeholder="npr. Maslinasto zelena"
                    />
                  </Field>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
              ...
              <Field label="Boja 1">
                <Input
                  name="colorPrimary"
                  list="article-colors"
                  defaultValue={product.colorPrimary ?? ""}
                />
              </Field>
              <Field label="Boja 2">
                <Input
                  name="colorSecondary"
                  list="article-colors"
                  defaultValue={product.colorSecondary ?? ""}
                />
              </Field>
            </div>
            <datalist id="article-colors">
              {lookupOptions("COLOR").map((value) => (
