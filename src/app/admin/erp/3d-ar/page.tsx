...
export default async function ProductArReport({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const filters = { product: field("product"), campaign: field("campaign"), content: field("content"), source: field("source"), device: field("device"), variant: field("variant") };
  const [report, products] = await Promise.all([getProductArReport(period, filters), db.product.findMany({ where: { analyticsEvents: { some: { type: "PRODUCT_AR" } } }, select: { id: true, name: true, sku: true }, orderBy: { name: "asc" } })]);
  ...
  return <>
    ...
    <div className="space-y-6 p-6 lg:p-8">
      <form className="grid gap-3 rounded-xl border border-border/60 bg-surface p-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="grid gap-1 text-xs">Poruka<select name="variant" defaultValue={filters.variant} className={selectClass}><option value="">A i B</option><option value="A">A · {AR_COPY.A}</option><option value="B">B · {AR_COPY.B}</option></select></label>
      ...
      <DataTable columns={columns} rows={rows.map((row, index) => ({ id: String(index), cells: {
        ...
        variant: <>{row.variant} · {AR_COPY[row.variant as "A" | "B"]}<div className="text-xs">{row.device}</div></>,
      ...
      <div className="space-y-2 text-sm text-ink-600">
        <p>A/B test: A — „{AR_COPY.A}”; B — „{AR_COPY.B}”. Dodela je nasumična 50/50 i ostaje ista u pregledaču. Poređenje kupovine obuhvata sve koji su videli AR dugme, uključujući one koji ga nisu koristili.</p>
