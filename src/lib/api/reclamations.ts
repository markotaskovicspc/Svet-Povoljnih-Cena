...
async function createReclamationRecord(
  ...
  if (
    ...
    order.userId !== options.expectedUserId
  ) {
  ...
  try {
    result = await db.$transaction(async (tx) => {
      // Lock the order as well as its line. A courier/admin status transition
      // cannot race a new reclamation after the delivery eligibility check.
