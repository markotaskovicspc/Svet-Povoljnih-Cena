/** Compress supplementary surface maps; preserve original photographed albedo bytes. */
import fs from 'node:fs/promises';
import path from 'node:path';
import sharp from 'sharp';
import { execFileSync } from 'node:child_process';
import assert from 'node:assert/strict';
const packages=[['cube-210030','cube-v6'],['x-desk-210027','x-desk-v2']];
const report=[];
async function jpeg(bytes,name) {
 const normal=/normal/i.test(name);
 const jpg=await sharp(bytes).jpeg({quality:normal?98:97,chromaSubsampling:'4:4:4'}).toBuffer();
 ...
 const psnr=10*Math.log10(255**2/(sq/a.length));assert.ok(psnr>42,`${name}: ${psnr} dB`);
...
for(const [folder,stem] of packages) {
 ...
 const work=path.resolve(`.tools/blendkit/package-${stem}`);await fs.mkdir(work,{recursive:true});execFileSync('unzip',['-qo',`public/models/${folder}/${stem}.usdz`,'-d',work]);
 const replacements={};
 for(const f of await fs.readdir(`${work}/textures`)) if(f.endsWith('.png')&&f.includes('blendkit')) {
...
await fs.writeFile('assets/material-library/compression-report.json',JSON.stringify(report,null,2)+'\n');
