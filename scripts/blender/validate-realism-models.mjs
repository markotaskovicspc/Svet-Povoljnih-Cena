...
const report=[];
for(const [folder,newName,expected] of [['cube-210030','cube-v7',[.8,.71,.8]],['x-desk-210027','x-desk-v3',[.7,.74,.48]]]){
 ...
 report.push({folder,bytes:bytes.length,sha256:sha(bytes),...g,basecolor_sha256:colors(m),generated_microdetail:true,embedded_textures:true,validation});
}
await fs.writeFile('assets/material-library/realism-v1/glb-validation.json',JSON.stringify(report,null,2)+'\n');console.log(report.map(({folder,bytes,triangles,size,validation})=>({folder,bytes,triangles,size,errors:validation.issues.numErrors,warnings:validation.issues.numWarnings})));
