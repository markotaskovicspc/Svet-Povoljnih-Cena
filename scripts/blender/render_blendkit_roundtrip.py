...
for folder,name,version,tag in [('cube-210030','cube','v6','source'),('x-desk-210027','x-desk','v2','product_part')]:
 for fmt in ['glb','usdz']:
  base=root/'assets'/folder
  bpy.ops.wm.open_mainfile(filepath=str(base/(name+'-blendkit.blend')))
  ...
  bpy.context.scene.render.filepath=str(base/'renders'/(version+'-blendkit-'+fmt+'-roundtrip.png'))
