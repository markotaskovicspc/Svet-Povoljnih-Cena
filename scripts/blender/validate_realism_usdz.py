...
for folder,name,size,albedo in [('cube-210030','cube-v7',[.8,.71,.8],'v7-generated-cloth-color.jpg'),('x-desk-210027','x-desk-v3',[.7,.74,.48],'v3-generated-oak-color.jpg')]:
 ...
 report={'folder':folder,'size_m':actual,'floor_y':low[1],'bytes':len(raw),'sha256':hashlib.sha256(raw).hexdigest(),'errors':list(checker.GetErrors()),'failed_checks':list(checker.GetFailedChecks()),'warnings':list(checker.GetWarnings()),'textures':textures,'reconstructed_basecolor_preserved':True}
 assert not report['errors'] and not report['failed_checks'],report
 reports.append(report)
(root/'assets/material-library/realism-v1/usdz-validation.json').write_text(json.dumps(reports,indent=2)+'\n');print(json.dumps(reports))
